...
LDFLAGS := $(LIBPATHS) -lcitro3d -lctru -lm -specs=3dsx.specs -g -marm -mthumb-interwork -march=armv6k -mtune=mpcore -mfloat-abi=hard -mtp=soft # -Wl,-Map,$(notdir $*.map)
  ...
  VITA_LIBS := -lSDL2 -lSDL2_mixer -lvitaGL -lvitashark -lmathneon \
               ...
               -lSceKernelDmacMgr_stub -lSceKernelModulemgr_stub -llibScePiglet_stub \
...
ifeq ($(TARGET_VITA),1)
  ...
  # Create param.sfo (ATTRIBUTE2=12 for unsafe homebrew; APP_VER=01.00 for PSN format)
  $(BUILD_DIR)/param.sfo:
  ...
  # Create SELF (eboot.bin) from VELF (generated by EXE rule below)
  # -c enables compression for smaller eboot.bin and faster install
  $(BUILD_DIR)/eboot.bin: $(BUILD_DIR)/$(TARGET).velf
	$(V)vita-make-fself -c $< $@

  # Build VPK (also depends on PNG assets to catch icon/bg/startup changes)
  $(VPK): $(BUILD_DIR)/eboot.bin $(BUILD_DIR)/param.sfo \
      $(SCE_SYS_DIR)/icon0.png \
      $(SCE_SYS_DIR)/livearea/contents/bg0.png \
      $(SCE_SYS_DIR)/livearea/contents/startup.png \
      $(SCE_SYS_DIR)/livearea/contents/template.xml
	$(V)vita-pack-vpk $@ \
  ...
  # Add VPK as extra prerequisite to `all:`
  all: $(VPK)
